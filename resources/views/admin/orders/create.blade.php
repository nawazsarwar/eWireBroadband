    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.orders.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('transacted_on') ? 'is-invalid' : '' }}" type="text" name="transacted_on" id="transacted_on" value="{{ old('transacted_on') }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('narration') ? 'is-invalid' : '' }}" type="text" name="narration" id="narration" value="{{ old('narration', '') }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('receivable') ? 'is-invalid' : '' }}" name="receivable_id" id="receivable_id">
                    @foreach($receivables as $id => $entry)
                        <option value="{{ $id }}" {{ old('receivable_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('merchant_order') ? 'is-invalid' : '' }}" type="text" name="merchant_order" id="merchant_order" value="{{ old('merchant_order', '') }}">
